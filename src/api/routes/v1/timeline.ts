...
async function handleTimeline(reply: FastifyReply, address: string) {
  try {
    const [databarnTimelineResponse, clmTimelineResponse] = await Promise.all([
...
export default async function (
  ...
  instance.get<TimelineRoute>('/timeline', { schema: timelineSchema }, async (request, reply) => {
    return handleTimeline(reply, request.query.address);
  });
